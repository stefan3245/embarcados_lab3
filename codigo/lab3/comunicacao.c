...
void task_comunicacao(void const *arg){
  ...
  int32_t temp_signal; //Variável para guardar um sinal a ser enviado
    ...
    if(UART_read(&c[0]) > 0) {
      if(is_number(c[0])){
        ...
        if(strlen(c) > 1){ //Caso a string do comando seja maior que 1, isso indica que é uma informação de posição do elevador
          ...
          switch (andar) {
          case 0:
            temp_signal = SIGNAL_CONTROLE_ANDAR_T;
            break;
          case 1:
            temp_signal = SIGNAL_CONTROLE_ANDAR_1;
            break;
          case 2:
            temp_signal = SIGNAL_CONTROLE_ANDAR_2;
            break;
          case 3:
            temp_signal = SIGNAL_CONTROLE_ANDAR_3;
            break;
          default:
            //TODO: tratar erro
            break;
        ...
        osSignalSet(tid_Controle, SIGNAL_CONTROLE_PORTAS_FECHADAS);
